<div class="container-fluid">
	<div class="row">
		<div class="col-xs-12 bg-inverse">
			<h1 class="text-uppercase text-center page-header">listes des articles</h1>
		</div>
	</div>
</div>

<div class="container">
	<div class="row">
		<div class="col-xs-6 col-xs-offset-3">
			<table class="table table-bordered table-striped table-hover">
				<thead>
					<tr>
						<th class="text-center text-uppercase">Date</th>
						<th class="text-center text-uppercase">Titre de l'article</th>
						<th class="text-center text-uppercase">Reediter</th>
						<th class="text-center text-uppercase">Supprimer</th>
					</tr>
				</thead>
				<tbody class="text-center list-inline">
					<tr>
						<td>01/01/2000</td>
						<td><a href="">articles 1</a></td>
						<td><button class="btn btn-md btn-warning" type="submit"><span class="glyphicon glyphicon-cog"></span></button></td>
						<td><button class="btn btn-md btn-danger"><span class="glyphicon glyphicon-remove"></span></button></td>
					</tr>
					<tr>
						<td>01/01/2000</td>
						<td><a href="">articles 1</a></td>
						<td><button class="btn btn-md btn-warning" type="submit"><span class="glyphicon glyphicon-cog"></span></button></td>
						<td><button class="btn btn-md btn-danger"><span class="glyphicon glyphicon-remove"></span></button></td>
					</tr>
					<tr>
						<td>01/01/2000</td>
						<td><a href="">articles 1</a></td>
						<td><button class="btn btn-md btn-warning" type="submit"><span class="glyphicon glyphicon-cog"></span></button></td>
						<td><button class="btn btn-md btn-danger"><span class="glyphicon glyphicon-remove"></span></button></td>
					</tr>
					<tr>
						<td>01/01/2000</td>
						<td><a href="">articles 1</a></td>
						<td><button class="btn btn-md btn-warning" type="submit"><span class="glyphicon glyphicon-cog"></span></button></td>
						<td><button class="btn btn-md btn-danger"><span class="glyphicon glyphicon-remove"></span></button></td>
					</tr>
					<tr>
						<td>01/01/2000</td>
						<td><a href="">articles 1</a></td>
						<td><button class="btn btn-md btn-warning" type="submit"><span class="glyphicon glyphicon-cog"></span></button></td>
						<td><button class="btn btn-md btn-danger"><span class="glyphicon glyphicon-remove"></span></button></td>
					</tr>
					<tr>
						<td>01/01/2000</td>
						<td><a href="">articles 1</a></td>
						<td><button class="btn btn-md btn-warning" type="submit"><span class="glyphicon glyphicon-cog"></span></button></td>
						<td><button class="btn btn-md btn-danger"><span class="glyphicon glyphicon-remove"></span></button></td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
</div>
